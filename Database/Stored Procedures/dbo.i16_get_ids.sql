...
BEGIN
  ...
  BEGIN 
    ...
    UPDATE DBO.i16 WITH  (tablockx, holdlock)
      SET base_id = base_id + @num_requested_ids
      WHERE num_ids = -1 AND id_type = @id_type
    DELETE FROM DBO.i16 WHERE id_type = @id_type and num_ids <> -1
  ...
  BEGIN
    ...
    BEGIN
      DECLARE I_cursor CURSOR FOR
        ...
        FROM DBO.i16 WITH (tablockx, holdlock)
      ...
      BEGIN
        ...
        BEGIN
          ...
          IF (@64bit_rowid = 0 AND (@base_id + @num_obtained_ids) > @max_int)
            SET @int_overflow = 1
          ELSE
            UPDATE DBO.i16 SET base_id = base_id + 1,
        ...
        BEGIN
          ...
          ELSE
            ...
            DELETE FROM DBO.i16 WHERE CURRENT OF I_cursor
        END
      END
    END
    CLOSE I_cursor
