...
BEGIN
 ...
 SELECT @count = COUNT(*) FROM tempdb.sys.objects SO INNER JOIN 
    ...
    dbo.SDE_object_locks OL ON PR.sde_id = OL.sde_id
    WHERE  OL.object_id = @objectIdVal AND
           OL.object_type = @objectTypeVal AND
           OL.application_id = @applicationIdVal AND
           (OL.sde_id <> @sdeIdVal OR
            OL.autolock = @autoLockVal) AND
           (OL.lock_type = 'E' /* E: Exclusive lock */ OR 
            @lockTypeVal = 'E')
...
GO
GRANT EXECUTE ON  [dbo].[SDE_object_check_lock_conflicts] TO [public]
GO
