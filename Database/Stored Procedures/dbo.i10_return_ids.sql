...
BEGIN
  ...
  BEGIN
    ...
    UPDATE DBO.i10 SET base_id = @base_id, last_id = (@base_id - 1)
       WHERE num_ids = -1 AND id_type = @id_type
  END
  ELSE
  BEGIN
    /* Insert a new fragment */
    INSERT INTO DBO.i10 (base_id, num_ids, id_type)
      VALUES (@base_id, @num_ids, @id_type)
  END
  COMMIT TRAN id_tran /* releases holdlock table lock */
END
GO
GRANT EXECUTE ON  [dbo].[i10_return_ids] TO [public] WITH GRANT OPTION
GO
