...
BEGIN
  ...
  BEGIN
    IF (@old_state_id != @current_state)
      INSERT INTO DBO.d35 VALUES (@old_state_id,@row_id,@current_state)
    ELSE
    BEGIN
      DELETE FROM DBO.a35 WHERE OID = @row_id AND SDE_STATE_ID = @current_state
    END
  END
  ELSE
  BEGIN -- editing default version
      ...
      (SELECT DISTINCT lineage_id
       ...
       WHERE lineage_name IN
        (SELECT lineage_name
         ...
         WHERE lineage_id IN
          (SELECT DELETED_AT
           FROM DBO.d35 WITH (TABLOCKX,HOLDLOCK)
    ...
    BEGIN
      ...
      BEGIN
        ...
        INSERT INTO DBO.d35 (DELETED_AT,SDE_DELETES_ROW_ID,SDE_STATE_ID) VALUES
         (@current_state, @row_id, @old_state_id)
      END
      ELSE
        DELETE FROM DBO.TRANSPORTATION_TOPOLOGY_POLY WHERE OID = @row_id
    END
...
CLOSE del_cursor
DEALLOCATE del_cursor
IF (SELECT COUNT (*) FROM dbo.SDE_mvtables_modified WHERE state_id = @current_state AND registration_id = 35) = 0
 AND @current_state > 0
EXECUTE dbo.SDE_mvmodified_table_insert 35, @current_state
END
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[v35_insert] ON [dbo].[Transportation_Topology_poly_evw] INSTEAD OF INSERT AS 
BEGIN
...
BEGIN
  SELECT @next_row_id = OID FROM inserted
    ...
    BEGIN
    EXECUTE DBO.i35_get_ids 2, 1, @next_row_id OUTPUT, @num_ids OUTPUT
    IF @num_ids > 1
    BEGIN
      SET @return_row_id = @next_row_id + 1
      SET @num_return_ids = @num_ids - 1
      EXECUTE DBO.i35_return_ids 2, @return_row_id, @num_return_ids
    END
  END
  ...
  IF @current_state = 0
  BEGIN
  INSERT INTO DBO.TRANSPORTATION_TOPOLOGY_POLY
...
DECLARE ins_cursor CURSOR FOR SELECT OID FROM #temp35 FOR UPDATE OF OID
OPEN ins_cursor
...
BEGIN
    ...
    BEGIN
      ...
      EXECUTE DBO.i35_return_ids 2, @return_row_id, @num_return_ids
    END
  UPDATE #temp35 SET OID = @next_row_id WHERE CURRENT OF ins_cursor
  FETCH NEXT FROM ins_cursor INTO @rowid
END
  -- If editing state 0, then the insert being performed
  -- must be written to the base table, not the adds table
...
INSERT INTO DBO.TRANSPORTATION_TOPOLOGY_POLY(
OID,OriginObjectClassName,OriginObjectID,DestinationObjectClassName,DestinationObjectID,RuleType,RuleDescription,isException,Shape,GDB_GEOMATTR_DATA) SELECT 
t.OID,t.OriginObjectClassName,t.OriginObjectID,t.DestinationObjectClassName,t.DestinationObjectID,t.RuleType,t.RuleDescription,t.isException,t.Shape,t.GDB_GEOMATTR_DATA FROM #temp35 t 
END
ELSE
BEGIN
INSERT INTO DBO.a35(
...
t.OID,t.OriginObjectClassName,t.OriginObjectID,t.DestinationObjectClassName,t.DestinationObjectID,t.RuleType,t.RuleDescription,t.isException,t.Shape,t.GDB_GEOMATTR_DATA,@current_state FROM #temp35 t 
END
CLOSE ins_cursor
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[v35_update]  ON [dbo].[Transportation_Topology_poly_evw] INSTEAD OF UPDATE AS 
BEGIN
IF @@rowcount = 0 RETURN
DECLARE @current_state BIGINT
DECLARE @ret INTEGER
...
OID,OriginObjectClassName,OriginObjectID,DestinationObjectClassName,DestinationObjectID,RuleType,RuleDescription,isException,Shape,GDB_GEOMATTR_DATA,SDE_STATE_ID)
    ...
    INSERT INTO DBO.d35 VALUES (@old_state_id, @old_row_id, @current_state)
    END
    ELSE
    BEGIN
     IF @old_spatial_column IS NOT NULL AND UPDATE(SHAPE)
UPDATE DBO.A35 SET OriginObjectClassName = i.OriginObjectClassName,OriginObjectID = i.OriginObjectID,DestinationObjectClassName = i.DestinationObjectClassName,DestinationObjectID = i.DestinationObjectID,RuleType = i.RuleType,RuleDescription = i.RuleDescription,isException = i.isException,Shape = i.Shape,GDB_GEOMATTR_DATA = NULL FROM DBO.A35  a INNER JOIN inserted i  ON (a.OID = i.OID)  AND (a.SDE_STATE_ID = i.SDE_STATE_ID) 
     WHERE a.OID = @old_row_id AND a.SDE_STATE_ID = @current_state
     ELSE
     UPDATE DBO.A35 SET OriginObjectClassName = i.OriginObjectClassName,OriginObjectID = i.OriginObjectID,DestinationObjectClassName = i.DestinationObjectClassName,DestinationObjectID = i.DestinationObjectID,RuleType = i.RuleType,RuleDescription = i.RuleDescription,isException = i.isException,Shape = i.Shape,GDB_GEOMATTR_DATA = i.GDB_GEOMATTR_DATA FROM DBO.A35  a INNER JOIN inserted i  ON (a.OID = i.OID)  AND (a.SDE_STATE_ID = i.SDE_STATE_ID) 
     WHERE a.OID = @old_row_id AND a.SDE_STATE_ID = @current_state
...
OID,OriginObjectClassName,OriginObjectID,DestinationObjectClassName,DestinationObjectID,RuleType,RuleDescription,isException,Shape,GDB_GEOMATTR_DATA,SDE_STATE_ID)
        ...
        INSERT INTO DBO.d35 (DELETED_AT,SDE_DELETES_ROW_ID,SDE_STATE_ID) VALUES (@current_state, @old_row_id, @old_state_id)
      END
      ELSE
      BEGIN
        IF @old_spatial_column IS NOT NULL AND UPDATE(SHAPE)
UPDATE DBO.TRANSPORTATION_TOPOLOGY_POLY SET OriginObjectClassName = i.OriginObjectClassName,OriginObjectID = i.OriginObjectID,DestinationObjectClassName = i.DestinationObjectClassName,DestinationObjectID = i.DestinationObjectID,RuleType = i.RuleType,RuleDescription = i.RuleDescription,isException = i.isException,Shape = i.Shape,GDB_GEOMATTR_DATA = NULL FROM DBO.TRANSPORTATION_TOPOLOGY_POLY  b INNER JOIN inserted i  ON (b.OID = i.OID) 
     WHERE b.OID = @old_row_id 
     ELSE
     UPDATE DBO.TRANSPORTATION_TOPOLOGY_POLY SET OriginObjectClassName = i.OriginObjectClassName,OriginObjectID = i.OriginObjectID,DestinationObjectClassName = i.DestinationObjectClassName,DestinationObjectID = i.DestinationObjectID,RuleType = i.RuleType,RuleDescription = i.RuleDescription,isException = i.isException,Shape = i.Shape,GDB_GEOMATTR_DATA = i.GDB_GEOMATTR_DATA FROM DBO.TRANSPORTATION_TOPOLOGY_POLY  b INNER JOIN inserted i  ON (b.OID = i.OID) 
     WHERE b.OID = @old_row_id 
