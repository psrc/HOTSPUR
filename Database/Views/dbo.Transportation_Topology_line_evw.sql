...
BEGIN
  ...
  BEGIN
    IF (@old_state_id != @current_state)
      INSERT INTO DBO.d38 VALUES (@old_state_id,@row_id,@current_state)
    ELSE
    BEGIN
      DELETE FROM DBO.a38 WHERE OID = @row_id AND SDE_STATE_ID = @current_state
    END
  END
  ELSE
  BEGIN -- editing default version
      ...
      (SELECT DISTINCT lineage_id
       ...
       WHERE lineage_name IN
        (SELECT lineage_name
         ...
         WHERE lineage_id IN
          (SELECT DELETED_AT
           FROM DBO.d38 WITH (TABLOCKX,HOLDLOCK)
    ...
    BEGIN
      ...
      BEGIN
        ...
        INSERT INTO DBO.d38 (DELETED_AT,SDE_DELETES_ROW_ID,SDE_STATE_ID) VALUES
         (@current_state, @row_id, @old_state_id)
      END
      ELSE
        DELETE FROM DBO.TRANSPORTATION_TOPOLOGY_LINE WHERE OID = @row_id
    END
...
CLOSE del_cursor
DEALLOCATE del_cursor
IF (SELECT COUNT (*) FROM dbo.SDE_mvtables_modified WHERE state_id = @current_state AND registration_id = 38) = 0
 AND @current_state > 0
EXECUTE dbo.SDE_mvmodified_table_insert 38, @current_state
END
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[v38_insert] ON [dbo].[Transportation_Topology_line_evw] INSTEAD OF INSERT AS 
BEGIN
...
BEGIN
  SELECT @next_row_id = OID FROM inserted
    ...
    BEGIN
    EXECUTE DBO.i38_get_ids 2, 1, @next_row_id OUTPUT, @num_ids OUTPUT
    IF @num_ids > 1
    BEGIN
      SET @return_row_id = @next_row_id + 1
      SET @num_return_ids = @num_ids - 1
      EXECUTE DBO.i38_return_ids 2, @return_row_id, @num_return_ids
    END
  END
  ...
  IF @current_state = 0
  BEGIN
  INSERT INTO DBO.TRANSPORTATION_TOPOLOGY_LINE
...
BEGIN
  ...
  SELECT OID,OriginObjectClassName,OriginObjectID,DestinationObjectClassName,DestinationObjectID,RuleType,RuleDescription,isException,Shape,GDB_GEOMATTR_DATA,SDE_STATE_ID
  FROM inserted
  DECLARE @col1 int
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[v38_update]  ON [dbo].[Transportation_Topology_line_evw] INSTEAD OF UPDATE AS 
BEGIN
IF @@rowcount = 0 RETURN
DECLARE @current_state BIGINT
DECLARE @ret INTEGER
...
DECLARE updt_cursor CURSOR FOR SELECT d.OID,d.SDE_STATE_ID,i.SHAPE,d.SHAPE,
 i.OID,
 i.OriginObjectClassName,
 i.OriginObjectID,
 i.DestinationObjectClassName,
 i.DestinationObjectID,
 i.RuleType,
 i.RuleDescription,
 i.isException,
 i.GDB_GEOMATTR_DATA
  ...
  ON i.OID = d.OID
DECLARE @upd_OID int
DECLARE @upd_OriginObjectClassName nvarchar(255) 
DECLARE @upd_OriginObjectID int
DECLARE @upd_DestinationObjectClassName nvarchar(255) 
DECLARE @upd_DestinationObjectID int
DECLARE @upd_RuleType nvarchar(255) 
DECLARE @upd_RuleDescription nvarchar(255) 
DECLARE @upd_isException int
DECLARE @upd_GDB_GEOMATTR_DATA varbinary(max) 
OPEN updt_cursor
FETCH NEXT FROM updt_cursor INTO @old_row_id, @old_state_id, @new_spatial_column, @old_spatial_column, @upd_OID, @upd_OriginObjectClassName, @upd_OriginObjectID, @upd_DestinationObjectClassName, @upd_DestinationObjectID, @upd_RuleType, @upd_RuleDescription, @upd_isException, @upd_GDB_GEOMATTR_DATA
WHILE @@FETCH_STATUS = 0
BEGIN
  IF @g_is_default = '0'
  BEGIN
...
INSERT INTO DBO.a38 (
OID,OriginObjectClassName,OriginObjectID,DestinationObjectClassName,DestinationObjectID,RuleType,RuleDescription,isException,Shape,GDB_GEOMATTR_DATA,SDE_STATE_ID)
         VALUES(  @upd_OID, @upd_OriginObjectClassName, @upd_OriginObjectID, @upd_DestinationObjectClassName, @upd_DestinationObjectID, @upd_RuleType, @upd_RuleDescription, @upd_isException, @new_spatial_column, @upd_GDB_GEOMATTR_DATA, @current_state)

     ELSE
INSERT INTO DBO.a38 (
OID,OriginObjectClassName,OriginObjectID,DestinationObjectClassName,DestinationObjectID,RuleType,RuleDescription,isException,Shape,GDB_GEOMATTR_DATA,SDE_STATE_ID)
          VALUES(  @upd_OID, @upd_OriginObjectClassName, @upd_OriginObjectID, @upd_DestinationObjectClassName, @upd_DestinationObjectID, @upd_RuleType, @upd_RuleDescription, @upd_isException, @new_spatial_column, NULL, @current_state)

     INSERT INTO DBO.d38 VALUES (@old_state_id, @old_row_id, @current_state)
    END
    ELSE
    BEGIN
     IF @old_spatial_column IS NOT NULL AND UPDATE(SHAPE)
UPDATE DBO.a38 SET OriginObjectClassName = @upd_OriginObjectClassName,OriginObjectID = @upd_OriginObjectID,DestinationObjectClassName = @upd_DestinationObjectClassName,DestinationObjectID = @upd_DestinationObjectID,RuleType = @upd_RuleType,RuleDescription = @upd_RuleDescription,isException = @upd_isException,Shape = @new_spatial_column,GDB_GEOMATTR_DATA = NULL  
WHERE OID = @old_row_id  AND SDE_STATE_ID = @current_state
     ELSE
     UPDATE DBO.a38 SET OriginObjectClassName = @upd_OriginObjectClassName,OriginObjectID = @upd_OriginObjectID,DestinationObjectClassName = @upd_DestinationObjectClassName,DestinationObjectID = @upd_DestinationObjectID,RuleType = @upd_RuleType,RuleDescription = @upd_RuleDescription,isException = @upd_isException,Shape = @new_spatial_column,GDB_GEOMATTR_DATA = @upd_GDB_GEOMATTR_DATA 
WHERE OID = @old_row_id  AND SDE_STATE_ID = @current_state
...
OID,OriginObjectClassName,OriginObjectID,DestinationObjectClassName,DestinationObjectID,RuleType,RuleDescription,isException,Shape,GDB_GEOMATTR_DATA,SDE_STATE_ID)
        ...
        INSERT INTO DBO.d38 (DELETED_AT,SDE_DELETES_ROW_ID,SDE_STATE_ID) VALUES (@current_state, @old_row_id, @old_state_id)
      END
      ELSE
      BEGIN
        IF @old_spatial_column IS NOT NULL AND UPDATE(SHAPE)
UPDATE DBO.TRANSPORTATION_TOPOLOGY_LINE SET OriginObjectClassName = @upd_OriginObjectClassName,OriginObjectID = @upd_OriginObjectID,DestinationObjectClassName = @upd_DestinationObjectClassName,DestinationObjectID = @upd_DestinationObjectID,RuleType = @upd_RuleType,RuleDescription = @upd_RuleDescription,isException = @upd_isException,Shape = @new_spatial_column,GDB_GEOMATTR_DATA = NULL  
WHERE OID = @old_row_id 
     ELSE
     UPDATE DBO.TRANSPORTATION_TOPOLOGY_LINE SET OriginObjectClassName = @upd_OriginObjectClassName,OriginObjectID = @upd_OriginObjectID,DestinationObjectClassName = @upd_DestinationObjectClassName,DestinationObjectID = @upd_DestinationObjectID,RuleType = @upd_RuleType,RuleDescription = @upd_RuleDescription,isException = @upd_isException,Shape = @new_spatial_column,GDB_GEOMATTR_DATA = @upd_GDB_GEOMATTR_DATA 
WHERE OID = @old_row_id 
