...
SET ANSI_NULLS ON
GO
CREATE VIEW [dbo].[evtNamedRoutes_evw] AS SELECT b.OBJECTID + 0 OBJECTID,b.NRouteID,b.MStart,b.MStop,b.OBJECTID - b.OBJECTID SDE_STATE_ID FROM DBO.EVTNAMEDROUTES b LEFT JOIN  (SELECT SDE_DELETES_ROW_ID,SDE_STATE_ID FROM DBO.d26 WHERE SDE_STATE_ID = 0 AND DELETED_AT IN (SELECT l.lineage_id FROM dbo.SDE_states s INNER JOIN dbo.SDE_state_lineages l ON l.lineage_name = s.lineage_name WHERE s.state_id = dbo.SDE_get_view_state() AND l.lineage_id <= s.state_id ) ) d ON b.OBJECTID = d.SDE_DELETES_ROW_ID WHERE d.SDE_STATE_ID IS NULL UNION ALL SELECT a.OBJECTID + 0 OBJECTID,a.NRouteID,a.MStart,a.MStop,a.SDE_STATE_ID FROM DBO.a26 a LEFT JOIN (SELECT SDE_DELETES_ROW_ID,SDE_STATE_ID FROM DBO.d26 WHERE DELETED_AT IN (SELECT l.lineage_id FROM dbo.SDE_states s INNER JOIN dbo.SDE_state_lineages l ON l.lineage_name = s.lineage_name WHERE s.state_id = dbo.SDE_get_view_state() AND l.lineage_id <= s.state_id ) ) d ON (a.OBJECTID = d.SDE_DELETES_ROW_ID) AND  (a.SDE_STATE_ID = d.SDE_STATE_ID) WHERE a.SDE_STATE_ID IN (SELECT l.lineage_id FROM dbo.SDE_states s INNER JOIN dbo.SDE_state_lineages l ON l.lineage_name = s.lineage_name WHERE s.state_id = dbo.SDE_get_view_state() AND l.lineage_id <= s.state_id ) AND d.SDE_STATE_ID IS NULL 
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[v26_delete]  ON [dbo].[evtNamedRoutes_evw] INSTEAD OF DELETE AS 
BEGIN
...
BEGIN
  ...
  BEGIN
    IF (@old_state_id != @current_state)
      INSERT INTO DBO.d26 VALUES (@old_state_id,@row_id,@current_state)
    ELSE
    BEGIN
      DELETE FROM DBO.a26 WHERE OBJECTID = @row_id AND SDE_STATE_ID = @current_state
    END
  END
  ELSE
  BEGIN -- editing default version
      ...
      (SELECT DISTINCT lineage_id
       ...
       WHERE lineage_name IN
        (SELECT lineage_name
         ...
         WHERE lineage_id IN
          (SELECT DELETED_AT
           FROM DBO.d26 WITH (TABLOCKX,HOLDLOCK)
    ...
    BEGIN
      ...
      BEGIN
        ...
        INSERT INTO DBO.d26 (DELETED_AT,SDE_DELETES_ROW_ID,SDE_STATE_ID) VALUES
         (@current_state, @row_id, @old_state_id)
      END
      ELSE
        DELETE FROM DBO.EVTNAMEDROUTES WHERE OBJECTID = @row_id
    END
...
CLOSE del_cursor
DEALLOCATE del_cursor
IF (SELECT COUNT (*) FROM dbo.SDE_mvtables_modified WHERE state_id = @current_state AND registration_id = 26) = 0
 AND @current_state > 0
EXECUTE dbo.SDE_mvmodified_table_insert 26, @current_state
END
GO
SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[v26_insert] ON [dbo].[evtNamedRoutes_evw] INSTEAD OF INSERT AS 
BEGIN
...
BEGIN
  SELECT @next_row_id = OBJECTID FROM inserted
    ...
    BEGIN
    EXECUTE DBO.i26_get_ids 2, 1, @next_row_id OUTPUT, @num_ids OUTPUT
    IF @num_ids > 1
    BEGIN
      SET @return_row_id = @next_row_id + 1
      SET @num_return_ids = @num_ids - 1
      EXECUTE DBO.i26_return_ids 2, @return_row_id, @num_return_ids
    END
  END
  ...
  IF @current_state = 0
  BEGIN
  INSERT INTO DBO.EVTNAMEDROUTES
  (OBJECTID,NRouteID,MStart,MStop)
  SELECT 
  @next_row_id,i.NRouteID,i.MStart,i.MStop  FROM inserted i
  END
  ELSE
  BEGIN
  INSERT INTO DBO.a26
  (OBJECTID,NRouteID,MStart,MStop,SDE_STATE_ID)
  SELECT 
  @next_row_id,i.NRouteID,i.MStart,i.MStop,@current_state  FROM inserted i
  END
END
ELSE
BEGIN
  ...
  SELECT OBJECTID,NRouteID,MStart,MStop,SDE_STATE_ID
  FROM inserted
  DECLARE @col1 int
  DECLARE @col2 int
  DECLARE @col3 numeric(8,2) 
  DECLARE @col4 numeric(8,2) 
  DECLARE @col5 bigint
  OPEN ins_cursor
  FETCH NEXT FROM ins_cursor INTO @col1,@col2,@col3,@col4,@col5
  WHILE @@FETCH_STATUS = 0
  BEGIN
    EXECUTE DBO.i26_get_ids 2, 1, @next_row_id OUTPUT, @num_ids OUTPUT
    IF @num_ids > 1
    ...
    BEGIN
      ...
      INSERT INTO DBO.EVTNAMEDROUTES
      (OBJECTID,NRouteID,MStart,MStop)
      VALUES (@next_row_id,@col2,@col3,@col4 )
    END
    ELSE
    BEGIN
      INSERT INTO DBO.a26
      (OBJECTID,NRouteID,MStart,MStop,SDE_STATE_ID)
      VALUES (@next_row_id,@col2,@col3,@col4,@current_state )
    END
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[v26_update]  ON [dbo].[evtNamedRoutes_evw] INSTEAD OF UPDATE AS 
BEGIN
IF @@rowcount = 0 RETURN
DECLARE @current_state BIGINT
DECLARE @ret INTEGER
...
DECLARE @upd_MStart numeric(8,2) 
DECLARE @upd_MStop numeric(8,2) 
OPEN updt_cursor
FETCH NEXT FROM updt_cursor INTO @old_row_id, @old_state_id, @upd_OBJECTID, @upd_NRouteID, @upd_MStart, @upd_MStop
WHILE @@FETCH_STATUS = 0
BEGIN
  IF @g_is_default = '0'
  BEGIN
    ...
    BEGIN
     INSERT INTO DBO.a26 (
OBJECTID,NRouteID,MStart,MStop,SDE_STATE_ID)
        VALUES(  @upd_OBJECTID, @upd_NRouteID, @upd_MStart, @upd_MStop, @current_state)

     INSERT INTO DBO.d26 VALUES (@old_state_id, @old_row_id, @current_state)
    END
    ELSE
    BEGIN
     UPDATE DBO.a26 SET NRouteID = @upd_NRouteID,MStart = @upd_MStart,MStop = @upd_MStop 
...
OBJECTID,NRouteID,MStart,MStop,SDE_STATE_ID)
        ...
        INSERT INTO DBO.d26 (DELETED_AT,SDE_DELETES_ROW_ID,SDE_STATE_ID) VALUES (@current_state, @old_row_id, @old_state_id)
      END
      ELSE
      BEGIN
        UPDATE DBO.EVTNAMEDROUTES SET NRouteID = @upd_NRouteID,MStart = @upd_MStart,MStop = @upd_MStop 
