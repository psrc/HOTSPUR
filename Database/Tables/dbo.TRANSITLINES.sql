...
GO
ALTER TABLE [dbo].[TRANSITLINES] ADD CONSTRAINT [g13_ck] CHECK (([SHAPE].[STSrid]=(2285)))
GO
ALTER TABLE [dbo].[TRANSITLINES] ADD CONSTRAINT [R42_pk] PRIMARY KEY CLUSTERED ([OBJECTID]) WITH (FILLFACTOR=100) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [G114LineID] ON [dbo].[TRANSITLINES] ([LineID]) WITH (FILLFACTOR=100) ON [PRIMARY]
GO
CREATE SPATIAL INDEX [S13_idx] ON [dbo].[TRANSITLINES] ([Shape]) USING geometry_auto_grid  WITH (BOUNDING_BOX = (1174245.1764705882, 22822.4705882353, 1459714.7058823528, 467867.9411764706), CELLS_PER_OBJECT = 16) ON [PRIMARY]
GO
